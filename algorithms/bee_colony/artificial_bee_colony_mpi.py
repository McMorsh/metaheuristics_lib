...
class ArtificialBeeColonyMPI(BaseAlgorithm):
    """
    Artificial Bee Colony with mpi4py.
    ...
    def initialize(self) -> None:
        ...
        self.fitness = mpi_evaluate_fitness(self.bees, self.fitness_function)

        if rank == 0:
            # Локальные лучшие и счётчики проб
            self.best_bee = self.fitness.copy()
            self.best_fitness = self.fitness.copy()
            # Счётчик неудачных попыток
            self.trial = np.zeros(self.n_bees, dtype=int)

            # Глобальные лучшие решения
            idx = np.argmin(self.fitness)
            self.global_best_bee = self.fitness[idx].copy()
            self.global_best_fitness = float(self.fitness[idx])
    ...
    def _neighbour(self, i):
        ...
        if rank == 0:
            ...
            if fit < self.best_fitness[i]:
                ...
                self.fitness[i] = fit
                self.best_bee[i] = new_solution.copy()
        ...
        if rank == 0:
            ...
            for i in range(self.n_bees):
                if self.trial[i] > self._limit:
                    ...
                    self.best_fitness[i] = new_fitness.copy()
                    self.trial[i] = 0

            # Обновляем глобальное лучшее решение
            idx = np.argmin(self.fitness)
            if self.fitness[idx] < self.global_best_fitness[idx]:
                self.global_best_bee = self.fitness[idx].copy()
                self.global_best_fitness = self.fitness[idx].copy()
