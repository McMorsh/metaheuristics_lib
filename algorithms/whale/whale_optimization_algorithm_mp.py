...
class WhaleOptimizationAlgorithmMP(BaseAlgorithm):
    """
    Whale Optimization Algorithm (WOA) с использованием multiprocessing.Pool
    ...
    def initialize(self) -> None:
        ...
        # Храним историю лучших значений фитнеса и китов
    ...
    def iterate(self) -> None:
        ...
        for i in range(self.n_whales):
            ...
            if np.random.rand() < 0.5:
                ...
                D = np.abs(C * self.best_whale - self.whales[i])
                new_pos = self.best_whale - A * D
            else:
                # Спиральное обновление
                D = np.abs(self.best_whale - self.whales[i])
